...
class ConnectionLogger(BaseLogger):
    """
    Defines loggers for shell connections.

    These loggers are identified in this way:

    ``test_suite.test_case.node_identifier.shell_name.connection``
    """
    def __init__(self, *args, **kwargs):
...
class LoggingManager(object):
    """
    Defines an object that creates loggers for Topology components.
    ...
    for each category by using the ``set_level`` method, this will change the
    level of all loggers in that category.

    All loggers are provided with a logging directory, taken from the value
    passed to ``--topology-log-dir`` option. This directory is used to store
    log files generated by the loggers.

    Loggers may also have a defined context, this is useful to provide a unique
    identifier for objects associated to a particular logger (like a log file,
    for example). This context should be the name of the *test suite* (the
    ``test_X.py`` file that holds one or more test cases).
    """
    def __init__(self):
        """
